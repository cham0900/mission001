...
const LEVELS = [
  {
    id: 0,
    name: "???",
    desc: "ภารกิจลับสุดยอด — ไม่มีข้อมูลเพิ่มเติม",
    time: 999,
    game: "secret",
  },
  {
  ...
  },
  {
    id: 3,
    name: "แฟนพันธ์แทร๊",
    desc: "วัดความเป็นแฟนคลับตัวจริ๊งงง",
    time: 60,
    game: "sequence",
  },
  {
    id: 4,
    name: "ภารกิจลับตามหาดวงดาว",
    desc: "กอบกู้โลกกันเถอะ!!!",
    time: 999,
    game: "platformer",
  },
  {
    id: 5,
    name: "การสอบเข้ากองบัญชาการลับ",
    desc: "เพื่อจะเป็นนักสืบของกองบัญชาการลับ",
    time: 20,
    game: "logic",
  },
  {
    id: 6,
    name: "แยกประเภทวัตถุต้องสงสัย?",
    desc: "ทักษะการสังเกตสิ่งของ นั้นมันระเบิดรึเปล่านะ",
    time: 17,
    game: "dragsort",
  },
  {
    id: 7,
    name: "นี้มันภาษาอะไรกันเนี๊ยะะ",
    desc: "เอเลี่ยนบุกโลกรึเปล่านะ",
    time: 35,
    game: "decode7",
  },
  {
    id: 8,
    name: "ฝึกคิดเลขเร็วกับเจ้าBI",
    desc: "เรียนคณิตกันเถอะ!",
    time: 25,
    game: "frequency",
  },
  {
    id: 9,
    name: "ออกเดินทางตามหา...",
    desc: "ทำไมมันน่ากลัวแบบนี้",
    time: 999,
    game: "cardriving",
  },
  {
...
function resumeBGM() {
  ...
  let vol = 0;
  const fade = setInterval(() => {
    vol = Math.min(0.77, vol + 0.03);
    bgm.volume = vol;
    if (vol >= 0.77) clearInterval(fade);
  }, 40);
}
...
function getHintMissionName(hint) {
  if (hint.level === "9b") return "BONUS ROUND";
  const mission = LEVELS.find((level) => level.id === hint.level);
  return mission ? mission.name : hint.label;
}
...
function updateMap() {
  ...
  const allMissionsComplete = LEVELS.filter((level) => level.id !== 0).every(
    (level) => done.includes(level.id),
  );
  ...
  HINTS.forEach((h) => {
    ...
    const missionLabel = isBonus
      ? "MISSION 09 ★ BONUS"
      : "MISSION " + String(h.level).padStart(2, "0");
    const missionName = getHintMissionName(h);
    if (collected) {
      if (h.isLink)
        div.innerHTML = `<div class="hint-level">◈ ${missionLabel} — ${missionName}</div><div class="hint-text"><a class="hint-link" href="${h.value}" target="_blank">${h.display}</a></div>`;
      else if (h.isImage)
        div.innerHTML = `<div class="hint-level">◈ ${missionLabel} — ${missionName}</div><img src="https://a.storyblok.com/f/161938/1200x676/d33939c7f6/counting-cards-in-blackjack.jpg" alt="Blackjack" style="max-width:100%;max-height:80px;display:block;margin-top:4px;border:1px solid var(--border)">`;
      else
        div.innerHTML = `<div class="hint-level">◈ ${missionLabel} — ${missionName}</div><div class="hint-text">${h.display}</div>`;
    } else {
      div.className = "hint-item hint-locked";
      div.innerHTML = `<div class="hint-level">◈ ${missionLabel} — ${missionName}</div><div class="hint-text">[ LOCKED ]</div>`;
    }
    hl.appendChild(div);
  });
  ...
  grid.innerHTML = "";
  LEVELS.forEach((lvl) => {
    if (lvl.id === 0 && !allMissionsComplete) return;
    const isDone = done.includes(lvl.id);
    const card = document.createElement("div");
    // Mission 000 gets special styling
    if (lvl.id === 0) {
      card.className = "level-card secret-card";
      card.innerHTML = `<div class="level-num" style="color:var(--warn)">MISSION 000</div><div class="level-name">${lvl.name}</div><div class="level-desc">${lvl.desc}</div><div class="level-status status-ready" style="color:var(--warn)">▶ ENTER</div>`;
    } else {
  ...
  if (done.includes(9)) {
    ...
    bonusCard.innerHTML = `<div class="level-num" style="color:var(--warn)">MISSION 09 ★ BONUS</div><div class="level-name" style="color:var(--warn)">BONUS ROUND</div><div class="level-desc">พบเจอสมบัติลับ ที่อาจเปลี่ยนชีวิตคุณได้</div><div class="level-status" style="color:${state.bonusUnlocked ? "var(--accent2)" : "var(--warn)"}">${state.bonusUnlocked ? "✓ BONUS COLLECTED" : "▶ BONUS AVAILABLE"}</div>`;
...
function startLevel(levelId) {
  ...
  document.getElementById("gameLevelNum").textContent =
    levelId === 0 ? "MISSION 000" : "MISSION " + String(levelId).padStart(2, "0");
...
function updateSideHints() {
  ...
  (state.collectedHints || []).forEach((h) => {
    ...
    div.innerHTML = `<div class="hint-level">MISSION ${String(h.level).padStart(2, "0")} — ${getHintMissionName(hd)}</div><div class="hint-text" style="font-size:11px">${hd.display.substring(0, 38)}${hd.display.length > 38 ? "..." : ""}</div>`;
    shl.appendChild(div);
  });
}
...
function openNameStage() {
  ...
  HINTS.forEach((h) => {
    ...
    if (h.isLink)
      ...
      div.innerHTML = `<div class="hint-level">◈ ${missionLabel} — ${missionName}</div><img src="https://a.storyblok.com/f/161938/1200x676/d33939c7f6/counting-cards-in-blackjack.jpg" alt="Blackjack" style="max-width:100%;max-height:90px;display:block;margin-top:6px;border:1px solid var(--border)">`;
    else
      div.innerHTML = `<div class="hint-level">◈ ${missionLabel} — ${missionName}</div><div class="hint-text">${h.display}</div>`;
    fhl.appendChild(div);
  });
...
// MISSION 000: SECRET — redirect to secret.html
function buildSecret(container, levelId) {
  document.getElementById("gameInstructions").textContent =
    "กำลังเข้าถึงระบบลับ...";
  container.innerHTML = `
    <div style="display:flex;flex-direction:column;align-items:center;justify-content:center;min-height:320px;gap:20px;width:100%;font-family:var(--mono)">
      <div style="font-size:48px;animation:blink 1s step-end infinite">◉</div>
      <div style="font-size:14px;color:var(--accent);letter-spacing:4px">ACCESSING CLASSIFIED SYSTEM...</div>
      <div style="font-size:11px;color:var(--text3);letter-spacing:2px">กำลังโหลด...</div>
    </div>
...
function buildReaction(container, levelId) {
  document.getElementById("gameInstructions").textContent =
    "คลิกเป้าหมายเรืองแสง 5 จุดก่อนที่เป้าหมายจะหายไป!";
...
function buildMemory(container, levelId) {
  document.getElementById("gameInstructions").textContent =
    "จดจำรูปแบบแล้วคลิกช่องตามลำดับให้ถูกต้อง เล่นทั้งหมด 3 รอบ";
  const SIZE = 4;
  let pattern = [],
    playerInput = [],
    phase = "show",
...
function buildSequenceOrder(container, levelId) {
  document.getElementById("gameInstructions").textContent =
    "ลากรายการไปเรียงลำดับให้ถูกต้อง";
  const correctOrder = [
    "Primo",
    "Secondo",
    "Terzo",
...
function buildPlatformer(container, levelId) {
  document.getElementById("gameInstructions").textContent =
    "ใช้ปุ่มลูกศรหรือ WASD เพื่อเคลื่อนที่และกระโดด เก็บดาวให้ครบทุกดวง!";
  const W = 640,
    H = 380;
  container.innerHTML = `<div style="font-family:var(--mono);font-size:13px;color:var(--text2);margin-bottom:10px;letter-spacing:2px">STARS: <span id="pStars" class="text-accent">0</span> / 8</div><canvas id="pfCanvas" width="${W}" height="${H}" style="max-width:100%;border:1px solid var(--border);image-rendering:pixelated"></canvas>`;
  ...
  const plats = [
    ...
    { x: 560, y: 140, w: 80, h: 14, hidden: true },
  ];
  const stars = [
    { x: 155, y: 228, c: false },
    { x: 345, y: 158, c: false },
    ...
    { x: 420, y: 308, c: false },
    { x: 620, y: 210, c: false },
    { x: 590, y: 120, c: false, hidden: true },
  ];
  const secretCorner = { x: 620, y: 320, w: 20, h: 20, discovered: false };
  const enemies = [
    { x: 250, y: 302, w: 20, h: 20, vx: 2, minX: 250, maxX: 380 },
    { x: 420, y: 232, w: 20, h: 20, vx: -2, minX: 420, maxX: 560 },
  ];
  ...
  function update() {
    ...
    if (!secretCorner.discovered && aabb(p, secretCorner)) {
      secretCorner.discovered = true;
      const secretStar = stars.find((s) => s.hidden);
      const secretPlatform = plats.find((pl) => pl.hidden);
      if (secretStar) secretStar.hidden = false;
      if (secretPlatform) secretPlatform.hidden = false;
      showNotification("SECRET PASSAGE REVEALED!");
      playBeep(880, 0.12);
    ...
    enemies.forEach((en) => {
      ...
      if (en.x < en.minX || en.x + en.w > en.maxX) {
        en.vx *= -1;
        en.x = Math.max(en.minX, Math.min(en.x, en.maxX - en.w));
      }
      if (aabb(p, en)) {
        resetPlatformer();
        showNotification("💀 YOU DIED! START OVER");
        playError();
      }
    });
  }
...
function buildLogic(container, levelId) {
  document.getElementById("gameInstructions").textContent =
    "ตอบคำถามตรรกะให้ถูกต้อง 3 ข้อเพื่อทำภารกิจให้สำเร็จ";
  const qs = [
    {
...
function buildDragSort(container, levelId) {
  document.getElementById("gameInstructions").textContent =
    "ลากรายการไปยังหมวดหมู่ที่ถูกต้อง จัดประเภททั้ง 6 รายการให้ครบ";
  const tokens = [
...
function buildDecode7(container, levelId) {
  document.getElementById("gameInstructions").textContent =
    "ถอดรหัสข้อความลับ แล้วพิมพ์คำตอบเพื่อส่งคำตอบ";
  const encoded = "WXQJWXQJVDKXU",
    answer = "TUNGTUNGSAHUR";
  container.innerHTML = `
  ...
  for (let i = 0; i < 13; i++) {
    ...
    el.addEventListener("input", (e) => {
      ...
      if (e.target.value && i < 12) inp.querySelectorAll("input")[i + 1].focus();
      playClick();
    });
...
function buildFrequency(container, levelId) {
  document.getElementById("gameInstructions").textContent =
    "แปลงเลขฐานสองแต่ละชุดเป็นเลขฐานสิบ แล้วตอบให้ถูกต้อง 3 ข้อ";
  const patterns = [
...
function buildCarDriving(container, levelId) {
  document.getElementById("gameInstructions").textContent =
    "ใช้ปุ่ม ← → หรือ A D เพื่อบังคับรถ เก็บดาวให้ครบและหลบสิ่งกีดขวาง มี 3 ชีวิต";
  const W = 640,
    H = 420;
  // Narrower road
  const RL = 140,
...
function buildCTF(container, levelId) {
  document.getElementById("gameInstructions").textContent =
    "กรอกคำตอบในช่องด้านล่าง แล้วกดส่งคำตอบเพื่อทำภารกิจให้สำเร็จ";
  container.innerHTML = `
...
function buildBonusGame(container, levelId) {
  document.getElementById("gameInstructions").textContent =
    "ตอบคำถามเกี่ยวกับ Katekyo Hitman Reborn! ให้ครบ 5 ข้อ เพื่อปลดล็อกคำใบ้โบนัส!";
